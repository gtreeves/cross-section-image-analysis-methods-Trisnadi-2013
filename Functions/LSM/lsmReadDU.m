...
while info.img_pos ~= 0
	...
	if ~isfield(info,'data')
		...
		if info.bits == 8
			info.data = zeros(info.height,info.width,...
				n_colors,info.lsm.DimensionZ,'uint8');
		elseif info.bits == 16
			info.data = zeros(info.height,info.width,...
				n_colors,info.lsm.DimensionZ,'uint16');
		else
	...
	if info.SamplesPerPixel == 1
		planeCnt = 1; [info,Image] = read_plane(info,planeCnt);
	else
	...
	if ~isempty(Image) && size(Image,1) == size(info.data,1)
